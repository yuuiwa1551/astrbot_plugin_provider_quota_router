...
`quota_state.json` 保存 pending reservation、短期 overlay、按 quota key 持久化的 cooldown、火山 provider 组的 403 熔断/探测状态，以及管理员错误告警限频时间。权威历史用量仍来自 AstrBot 原生 `data_v4.db` 的 `provider_stats` 表。
