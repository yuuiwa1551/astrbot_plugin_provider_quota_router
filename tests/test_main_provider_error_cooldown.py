...
class MainProviderErrorCooldownTests(unittest.IsolatedAsyncioTestCase):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            await plugin.opencode_quota_guard_error(
                ...
                ProviderAttemptTimeoutError(
                    "relay/model-a first response timed out after 20 seconds"
                ),
            )
    ...
    async def test_second_local_attempt_timeout_opens_short_circuit(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            plugin = object.__new__(ProviderQuotaRouterPlugin)
            plugin.settings = RouterSettings(
                provider_error_cooldown_enabled=True,
            ...
            await plugin.opencode_quota_guard_error(provider, error)
            await plugin.opencode_quota_guard_error(provider, error)
    ...
    async def test_success_resets_local_attempt_timeout_streak(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            plugin = object.__new__(ProviderQuotaRouterPlugin)
            plugin.settings = RouterSettings(
                provider_error_cooldown_enabled=True,
            ...
            await plugin.opencode_quota_guard_error(provider, error)
            await plugin.opencode_quota_guard_success(provider)
            await plugin.opencode_quota_guard_error(provider, error)
