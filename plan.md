...
目标：保留单次请求 20 秒首响应止损，同时避免把一次生成偏慢误判成 Provider 全局故障并冷却 30 分钟。

状态：v0.12.1 已完成并部署到实时 AstrBot；容器内 89 项测试通过，认证状态 API 显示新策略与两个 guard 生效，GLM 5.2 和 Doubao 2.0 Mini 真实 Provider 测试均为 `available`，详见 `15期plan.md`。

计划交付：

- 区分插件本地首响应预算耗尽与 Provider/SDK 原生超时。
- 首次本地预算耗尽只结束当前尝试并 fallback，不写持久模型健康状态。
