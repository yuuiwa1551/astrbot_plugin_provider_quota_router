...
这个插件面向“单一模型每日免费额度”场景，例如火山引擎开发者活动。当前默认业务规则是北京时间 11:00 切换额度窗口、火山模型达线后冷却 24 小时；`deepseek/` 自有 API 不参与额度限制。
...
- 默认把 `deepseek/` provider 视为不限额，只统计用量，不阻断也不预占。
...
| `unlimited_provider_prefixes` | `["deepseek/"]` | 不参与额度限制的 Provider ID 前缀 |
| `volcengine_403_circuit_enabled` | `true` | 是否启用火山 403 组级熔断 |
| `volcengine_provider_source_ids` | `["openai"]` | 用于识别火山模型组的 provider source ID |
...
`quota_state.json` 保存 pending reservation、短期 overlay、按 quota key 持久化的 cooldown，以及火山 provider 组的 403 熔断/探测状态。权威历史用量仍来自 AstrBot 原生 `data_v4.db` 的 `provider_stats` 表。
...
受控模型在 `effective_tokens + request_reservation + safety_buffer >= daily_limit` 时进入冷却。模型恢复必须同时满足：已经进入新的 11:00 日窗口，并且从达线时刻开始的 24 小时冷却已经结束。若模型在 15:00 达线，次日 11:00 仍会跳过，直到 15:00 才恢复。不限额前缀只绕过额度与冷却判断，请求模态检查仍然有效。
...
`volcengine_403_circuit_enabled=true` 时，插件会在一次受控 Agent 请求最终返回 403 后持久化整组火山熔断。触发该错误的本次请求仍按 AstrBot 当时的错误处理结束；从下一次请求开始，路由器会跳过全部火山候选并继续选择非火山 fallback。30 分钟到期后，后台只从仍满足单模型 token 安全线的火山文本模型中随机选一个，使用单次重试的最小提示做半开探测；成功清除熔断，失败则重新冷却 30 分钟。用户请求不会被拿来当恢复探针。
